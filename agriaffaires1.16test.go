...
func scrapeAllPages(baseURL string, pageLimit int) []*Tractor {
	var allTractors []*Tractor
	pageNum := 1

	for pageNum <= pageLimit {
		url := fmt.Sprintf("%s%d/farm-tractor.html", baseURL, pageNum)
...
func main() {
	baseURL := "https://www.agriaffaires.co.uk/used/"
	pageLimit := 5
	tractors := scrapeAllPages(baseURL, pageLimit)
