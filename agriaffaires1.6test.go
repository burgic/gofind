...
func scrapePage(url string) (*Tractor, error) {
    ...
    vatInfo := strings.TrimSpace(vatElement.Text())

    // Remove any thousand separators
    displayedPrice = strings.ReplaceAll(displayedPrice, ",", "")
    ...
    locationElement := doc.Find(".block--contact-desktop .u-bold").FilterFunction(func(i int, s *goquery.Selection) bool {
        return strings.TrimSpace(s.Text()) != ""
    }).Last()
    tractor.Location = strings.TrimSpace(locationElement.Text())
    ...
    // Debug information
    tractor.DebugInfo = map[string]string{
        "DisplayedPriceElement":  displayedPrice,
        "DisplayedCurrencyElement": displayedCurrency,
        "VATInfo":                vatInfo,
        "ReferencePriceElement":  tractor.ReferencePrice,
        "ReferenceCurrencyElement": tractor.ReferenceCurrency,
        "DealerElement":          tractor.Dealer,
        "LocationElement":        tractor.Location,
        "PhoneElement":           tractor.PhoneNumber,
    }
...
func main() {
    ...
    fmt.Printf("Phone Number (encoded): %s\n", tractor.PhoneNumber)
