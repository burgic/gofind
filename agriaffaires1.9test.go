...
// Function to scrape the page and return the tractor details
func scrapePage(url string) (*Tractor, error) {
    ...
    doc.Find("table tbody tr").Each(func(i int, s *goquery.Selection) {
        ...
        // Skip if the key or value is empty or irrelevant
        if key == "" || value == "" {
            return
        }
    ...
    // Extract the displayed price (ignore currency handling)
    priceElement := doc.Find(".js-priceToChange").First()
    displayedPrice := strings.TrimSpace(priceElement.Text())

    // Just store the displayed price as-is, without appending the currency
    tractor.DisplayedPrice = displayedPrice
