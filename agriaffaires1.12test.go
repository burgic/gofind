...
func scrapePage(url string) (*Tractor, error) {
    ...
    // Extract additional price information (e.g., "Observe prices")
    additionalInfo := strings.TrimSpace(priceBlock.Find(".js-quotation-hidden").Text())
    if additionalInfo != "" {
        tractor.PriceAdditionalInfo = additionalInfo
    }
...
func saveToCSV(tractors []*Tractor) error {
	...
	// Write the header
	headers := []string{
		"URL", "Displayed Price", "Category", "Make", "Model", "Dealer", "Location", "Phone Number",
	}
	...
	for _, tractor := range tractors {
		record := []string{
			tractor.URL, tractor.DisplayedPrice, tractor.Category, tractor.Make, tractor.Model,
			tractor.Dealer, tractor.Location, tractor.PhoneNumber,
		}
