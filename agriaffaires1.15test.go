...
// Scrapes the listing page to get tractor URLs and prices
func scrapeListing(url string) ([]string, bool, error) {
    ...
    doc.Find("a.listing-block__link").Each(func(i int, s *goquery.Selection) {
    ...
    // Check for the next page using the pagination link
    var hasNextPage bool
    doc.Find(".pagination__link--next").Each(func(i int, s *goquery.Selection) {
        hasNextPage = true
    })

    log.Printf("Found %d tractor URLs on listing page", len(urls))
    return urls, hasNextPage, nil
}
...
func scrapePage(url string) (*Tractor, error) {
	...
	priceBlock := doc.Find(".h1-like.u-bold")
	...
	log.Printf("Successfully scraped tractor page: %s", url)
	return tractor, nil
}

// Scrape all pages from the base URL
// Scrape up to 5 pages from the base URL for testing purposes
func scrapeAllPages(baseURL string) []*Tractor {
    var allTractors []*Tractor
    pageNum := 1
    maxPages := 5  // Limit scraping to first 5 pages
    for {
        // Stop after scraping 5 pages
        if pageNum > maxPages {
            log.Printf("Reached the maximum number of test pages (%d). Stopping.", maxPages)
            break
        }

        url := baseURL
		if pageNum > 1{
			url = fmt.Sprintf("%s?p=%d", baseURL, pageNum)
		}
        log.Printf("Scraping listing page: %s", url)

        urls, hasNextPage, err := scrapeListing(url)
        if err != nil {
        ...
        for _, url := range urls {
            ...
            if err != nil {
                log.Printf("Error scraping tractor page %s: %v", url, err)
                continue
            }
            allTractors = append(allTractors, tractor)
        }

        if !hasNextPage {
            log.Printf("No next page found. Stopping.")
            break
        }

        pageNum++
        randomDelay() // Random delay between page scrapes
    }
    return allTractors
}


// Save tractor data to a CSV file
func saveToCSV(tractors []*Tractor) error {
	...
	// Write the header
	headers := []string{
		"URL", "Category", "Type of ad", "Reference", "Make", "Model", "Status", "Power",
	...
	for _, tractor := range tractors {
		record := []string{
			tractor.URL, tractor.Category, tractor.AdType, tractor.Reference, tractor.Make, tractor.Model,
...
func main() {
	baseURL := "https://www.agriaffaires.co.uk/used/&d/farm-tractor.html"
	tractors := scrapeAllPages(baseURL)
	if err := saveToCSV(tractors); err != nil {
		fmt.Printf("Error saving data to CSV: %v\n", err)
	}

	var wg sync.WaitGroup
	results := make(chan *Tractor, len(tractors))

	for _, url := range tractors {
		wg.Add(1)
		go func(url string) {
			defer wg.Done()
			tractor, err := scrapePage(url)
	...
	var finalTractors []*Tractor
	for tractor := range results {
		finalTractors = append(finalTractors, tractor)
	}

	// Save the final scraped data to CSV
	if err := saveToCSV(finalTractors); err != nil {
		fmt.Printf("Error saving data to CSV: %v\n", err)
	}
}
